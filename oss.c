...
// Global declaration of the clock variable
struct Clock *sys_clock;
...
int main(int argc, char *argv[]) {
 ...
 // Attach to shared memory for clock
	sys_clock = (struct Clock *)shmat(shmid_sys_clock, NULL, 0);
    	if (sys_clock == (void *) -1) {
...
// Print message indicating successful initialization of process table shared memory
	...
	increment_clock(sys_clock, 0, interval * 1000000); // Increment by 'interval' milliseconds// Check if any child process has terminated
        int pid = waitpid(-1, NULL, WNOHANG);
        if (pid > 0) {
    // Update process table
    for (int i = 0; i < MAX_PROCESSES; i++) {
        if (process_table[i].occupied && process_table[i].pid == pid) {
            // Mark the entry as unoccupied
            process_table[i].occupied = false;
            process_table[i].pid = 0;
            process_table[i].startSeconds = 0;
            process_table[i].startNano = 0;
